...
class CitadineService {
  constructor(pool) {
    this.pool = pool;
  }

  // Création de la table formules_citadine si elle n'existe pas
  async createTable() {
    try {
  ...
  // Récupérer toutes les formules citadine
  async getAllFormules() {
    try {
      const result = await this.pool.query('SELECT * FROM formules_citadine ORDER BY created_at DESC');
      return result.rows;
    } catch (err) {
  ...
  async addFormule(formuleData) {
    try {
      ...
      const result = await this.pool.query(
        'INSERT INTO formules_citadine (nom, prix, duree, icone, services, lavage_premium, lavage_premium_prix) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
  ...
  // Supprimer une formule citadine
  async deleteFormule(id) {
    try {
      const result = await this.pool.query('DELETE FROM formules_citadine WHERE id = $1 RETURNING *', [id]);
  ...
  async updateFormule(id, formuleData) {
    try {
      ...
      const result = await this.pool.query(
        'UPDATE formules_citadine SET nom = $1, prix = $2, duree = $3, icone = $4, services = $5, lavage_premium = $6, lavage_premium_prix = $7 WHERE id = $8 RETURNING *',
  ...
  async updateLavagePremiumPrice(id, lavage_premium_prix) {
    try {
      const result = await this.pool.query(
        'UPDATE formules_citadine SET lavage_premium_prix = $1 WHERE id = $2 RETURNING *',
  ...
  async getFormuleById(id) {
    try {
      const result = await this.pool.query('SELECT * FROM formules_citadine WHERE id = $1', [id]);
  ...
  async getFormulesCount() {
    try {
      const result = await this.pool.query('SELECT COUNT(*) FROM formules_citadine');
...
module.exports = CitadineService; 