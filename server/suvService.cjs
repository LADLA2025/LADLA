...
class SuvService {
  ...
  async addFormule(formuleData) {
    try {
      ...
      const result = await this.pool.query(
        'INSERT INTO formules_suv (nom, prix, duree, icone, services) VALUES ($1, $2, $3, $4, $5) RETURNING *',
  ...
  async updateFormule(id, formuleData) {
    try {
      ...
      const result = await this.pool.query(
        'UPDATE formules_suv SET nom = $1, prix = $2, duree = $3, icone = $4, services = $5 WHERE id = $6 RETURNING *',
